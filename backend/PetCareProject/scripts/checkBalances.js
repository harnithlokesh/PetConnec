...
async function main() {
  ...
  // Check balances
  const reserveBalance = await dummyToken.balanceOf(reserveWallet);
  const userBalance = await dummyToken.balanceOf(deployer.address);

  console.log("Reserve wallet balance:", hre.ethers.utils.formatEther(reserveBalance), "ETN");
  console.log("User wallet balance:", hre.ethers.utils.formatEther(userBalance), "ETN");
}
